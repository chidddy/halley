...
void YAMLConvert::emitNode( const ConfigNode& node, YAML::Emitter& emitter, const EmitOptions& options )
{
    ...
    {
        const auto vec = node.asVector3i();
        emitter << YAML::Flow << YAML::BeginSeq << vec.x << vec.y << vec.z << YAML::EndSeq;
    }
        return;

    case ConfigNodeType::Int4:
    {
        const auto vec = node.asVector4i();
        emitter << YAML::Flow << YAML::BeginSeq << vec.x << vec.y << vec.z << vec.w << YAML::EndSeq;
    }
        return;
    ...
    {
        const auto vec = node.asVector3f();
        emitter << YAML::Flow << YAML::BeginSeq << vec.x << vec.y << vec.z << YAML::EndSeq;
    }
        return;
    ...
    {
        const auto vec = node.asVector4f();
        emitter << YAML::Flow << YAML::BeginSeq << vec.x << vec.y << vec.z << vec.w << YAML::EndSeq;
    }
        return;
