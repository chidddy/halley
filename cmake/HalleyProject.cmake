...
function(halleyProjectServer name sources headers srv_sources srv_headers genDefinitions targetDir)
	set(EMBED ${HALLEY_PROJECT_EMBED})
	set(HALLEY_PROJECT_EMBED 0)
	...
	set(srv_proj_sources ${srv_sources} ${${name}_sources_gen} ${${name}_sources_systems})
	set(srv_proj_headers ${srv_headers} ${${name}_headers_gen} ${genDefinitions})

	assign_source_group(${proj_sources})
	assign_source_group(${proj_headers})
	assign_source_group(${srv_proj_sources})
	assign_source_group(${srv_proj_headers})
	...
	elseif (HALLEY_MONOLITHIC)
		...
		add_library(${name}-srv-game STATIC ${srv_proj_sources} ${srv_proj_headers})
		add_library(${name}-srv-dll SHARED ${HALLEY_PATH}/src/entry/halley_dll_entry.cpp)

		if (BUILD_MACOSX_BUNDLE)
			add_executable(${name} MACOSX_BUNDLE ${HALLEY_PATH}/src/entry/halley_exe_entry.cpp)
		else()
			add_executable(${name} WIN32 ${HALLEY_PATH}/src/entry/halley_exe_entry.cpp)
			add_executable(${name}-srv WIN32 ${HALLEY_PATH}/src/entry/halley_exe_entry.cpp)
		endif()
		#set_target_properties(${name}-exe PROPERTIES OUTPUT_NAME ${name})
		...
		# Setup a gamebins target for external building
		add_custom_target(${name}-gamebins DEPENDS ${name} ${name}-dll)
		add_custom_target(${name}-srv-gamebins DEPENDS ${name}-srv ${name}-srv-dll)
	endif()
	...
	if (HALLEY_MONOLITHIC)
		target_link_libraries(${name} ${LINK_LIBRARIES})
		target_link_libraries(${name}-srv ${LINK_LIBRARIES})
	else ()
		target_link_libraries(${name} ${name}-game ${LINK_LIBRARIES})
		target_link_libraries(${name}-dll ${name}-game ${LINK_LIBRARIES})

		target_link_libraries(${name}-srv ${name}-srv-game ${LINK_LIBRARIES})
		target_link_libraries(${name}-srv-dll ${name}-srv-game ${LINK_LIBRARIES})
	endif()

	if (NOT ${CMAKE_SYSTEM_NAME} MATCHES "WindowsStore")
		set_target_properties(${name} PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
		set_target_properties(${name}-srv PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
		if (TARGET ${name}-game)
			set_target_properties(${name}-game PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
		endif()
		if (TARGET ${name}-dll)
		...
		if (TARGET ${name}-srv-game)
		set_target_properties(${name}-srv-game PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
	endif()
	if (TARGET ${name}-srv-dll)
		set_target_properties(${name}-srv-dll PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
	endif()
	endif ()
...
endfunction(halleyProjectServer)
